...
from losses import contrastive_loss, negative_sampling_contrastive_loss,TripletLoss
from models import MLPModel, GCNModel, AttentionModel,cross_modal_discriminator,TransformerRMLPModel
...
parser.add_argument('--bert_lr', type=float, nargs='?', default=3e-5,
                    help='Size of data batch.')
parser.add_argument('--lamda', type=float, nargs='?', default=0.002,
                    help='the parameter lamda of adversarial loss.')

args = parser.parse_args()
# args.data='/fred/oz064/wenyuzhao/Experiments/text2mol/data'
# args.output_path='/fred/oz064/wenyuzhao/ExperimentalResults/OnlineText2Melo/GraphTransformerGANRMLPTrip60Lamda0.002head2/'
args.data='E:\\text2mol\\text2mol-master\\data'
args.output_path='E:\\text2mol\\onlinecode\\GCN1_output\\'
args.model='GCN'
args.batch_size=32
args.epochs=60
args.lamda=0.002
...
if MODEL == "MLP":
    ...
    model = TransformerRMLPModel(num_node_features=graph_batcher_tr.dataset.num_node_features, ninp=768, nhid=600, nout=300,graph_hidden_channels=600, heads=2)
    cs_align_model = cross_modal_discriminator(input_dim=300)
...
ad_optimizer = optim.Adam(cs_align_model.parameters(), lr=init_lr, betas=(0.5, 0.999))
...
if MODEL == "Attention" or MODEL=="GCN":
    ...
    tmp2 = cs_align_model.set_device(device)
else:
    tmp = model.to(device)

lossfun = TripletLoss(margin=0.35)
...
for epoch in range(epochs):
    ...
    for i, d in enumerate(training_generator):
        ...
        if MODEL == "MLP":
            ...
            # loss = contrastive_loss(text_out, chem_out).to(device)
            real_validity = cs_align_model(text_out)
            fake_validity = cs_align_model(chem_out)
            gradient_penalty = compute_gradient_penalty(cs_align_model, text_out, chem_out)
            loss_discriminator = -torch.mean(real_validity) + torch.mean(fake_validity) + 10 * gradient_penalty
            loss_discriminator = loss_discriminator.to(device)
            # print("train loss-discri:", loss_discriminator)
            g_fake_validity = cs_align_model(chem_out)
            loss_generator = -torch.mean(g_fake_validity.detach()).to(device)

            triploss = lossfun(text_out, chem_out).to(device)
            # print("train loss:", loss_generator, triploss)
            loss = triploss + lamda * loss_generator
            running_loss += loss.item()
    ...
    with torch.set_grad_enabled(True):
        ...
        for i, d in enumerate(validation_generator):
            ...
            if MODEL == "MLP":
                ...
                loss_discriminator = -torch.mean(real_validity) + torch.mean(fake_validity) + 10 * gradient_penalty
                loss_discriminator = loss_discriminator.to(device)
                # print("val loss-discri:", loss_discriminator)

                g_fake_validity = cs_align_model(chem_out)
                # g_fake_validity = Variable(g_fake_validity, requires_grad = True)
                loss_generator = -torch.mean(g_fake_validity.detach()).to(device)

                triploss = lossfun(text_out, chem_out).to(device)
                # print("val loss:", loss_generator, triploss)
                loss = triploss + lamda * loss_generator
