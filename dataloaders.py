...
class GenerateData():
  ...
  def load_substructures(self):
    ...
    with open(self.path_molecules, encoding='utf8') as f:
      for line in f:
        spl = line.split(":")
        cid = spl[0]
        tokens = spl[1].strip()
  ...
  def prep_text_tokenizer(self):
    ...
    self.text_tokenizer = BertTokenizerFast.from_pretrained(modelname, config=config)
  ...
  def store_descriptions(self):
    ...
    with open(self.path_train, encoding='utf8') as f:
    ...
    with open(self.path_val, encoding='utf8') as f:
      reader = csv.DictReader(f, delimiter="\t", quoting=csv.QUOTE_NONE, fieldnames = ['cid', 'mol2vec', 'desc'])
      for n, line in enumerate(reader):
        self.descriptions[line['cid']] = line['desc']
        self.mols[line['cid']] = line['mol2vec']
    ...
    self.test_cids = []
    #get test set cids...
    with open(self.path_test, encoding='utf8') as f:
      reader = csv.DictReader(f, delimiter="\t", quoting=csv.QUOTE_NONE, fieldnames = ['cid', 'mol2vec', 'desc'])
      for n, line in enumerate(reader):
        self.descriptions[line['cid']] = line['desc']
        self.mols[line['cid']] = line['mol2vec']
...
def get_dataloader(data_generator, params):
    ...
    # print("set: ", training_set.length,validation_set.length,test_set.length)
...
class MoleculeGraphDataset(GeoDataset):
    def __init__(self, root, cids, data_path, gt, transform=None, pre_transform=None):
        ...
        for raw_path in self.raw_paths:
            # cid = int(raw_path.split('/')[-1][:-6])   # Windows下分割错误,提取图结构文件的文件名并转换为int类型作为cid存储
            cid = int(osp.splitext(osp.basename(raw_path))[0])  # Get the file name without extension
    ...
    def process_graph(self, raw_path):
      ...
      with open(raw_path, 'r', encoding='utf8') as f:
        ...
        print("length:",len(edge_index), len(x))
...
class CustomGraphCollater(object):
    ...
    def __call__(self, cids):
        ...
        # return self.collate([self.dataset.get_cid(int(float(cid))) for cid in cids])
...
def get_graph_data(data_generator, graph_data_path):
    ...
    # print("mg data: ", mg_data_tr,mg_data_val,mg_data_test)
