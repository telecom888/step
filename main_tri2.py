...
from dataloaders import get_dataloader, GenerateData, get_graph_data, get_attention_graph_data, GenerateDataAttention, \
...
# 定义双向三元组损失函数，并利用 f_out 参数进行计算
def bidirectional_triplet_loss(text_out, chem_out, f_out, margin=0.3, lambda_param=2e-3):
    batch_size = text_out.size(0)

    # 1. 计算原始模态损失 (text_out vs. chem_out)
    dist_matrix_orig = F.pairwise_distance(text_out.unsqueeze(1).expand(-1, batch_size, -1),
                                           chem_out.unsqueeze(0).expand(batch_size, -1, -1), p=2)
    pos_dist_orig = torch.diag(dist_matrix_orig)
    neg_dist_text_orig = dist_matrix_orig.clone()
    neg_dist_chem_orig = dist_matrix_orig.clone()
    torch.diagonal(neg_dist_text_orig).fill_(float('inf'))
    torch.diagonal(neg_dist_chem_orig).fill_(float('inf'))
    neg_dist_text_min_orig, _ = neg_dist_text_orig.min(dim=1)
    neg_dist_chem_min_orig, _ = neg_dist_chem_orig.min(dim=0)
    loss_text_orig = F.relu(pos_dist_orig - neg_dist_text_min_orig + margin).mean()
    loss_chem_orig = F.relu(pos_dist_orig - neg_dist_chem_min_orig + margin).mean()
    loss_orig = (loss_text_orig + loss_chem_orig) / 2

    # 2. 计算融合模态与文本模态的损失 (f_out vs. text_out)
    dist_matrix_c_text = F.pairwise_distance(f_out.unsqueeze(1).expand(-1, batch_size, -1),
                                             text_out.unsqueeze(0).expand(batch_size, -1, -1), p=2)
    pos_dist_c_text = torch.diag(dist_matrix_c_text)
    neg_dist_c_text = dist_matrix_c_text.clone()
    neg_dist_text_c = dist_matrix_c_text.clone()
    torch.diagonal(neg_dist_c_text).fill_(float('inf'))
    torch.diagonal(neg_dist_text_c).fill_(float('inf'))
    neg_dist_c_min_text, _ = neg_dist_c_text.min(dim=1)
    neg_dist_text_min_c, _ = neg_dist_text_c.min(dim=0)
    loss_c_text_1 = F.relu(pos_dist_c_text - neg_dist_c_min_text + margin).mean()
    loss_c_text_2 = F.relu(pos_dist_c_text - neg_dist_text_min_c + margin).mean()
    loss_c_text = (loss_c_text_1 + loss_c_text_2) / 2

    # 3. 计算融合模态与分子模态的损失 (f_out vs. chem_out)
    dist_matrix_c_chem = F.pairwise_distance(f_out.unsqueeze(1).expand(-1, batch_size, -1),
                                             chem_out.unsqueeze(0).expand(batch_size, -1, -1), p=2)
    pos_dist_c_chem = torch.diag(dist_matrix_c_chem)
    neg_dist_c_chem = dist_matrix_c_chem.clone()
    neg_dist_chem_c = dist_matrix_c_chem.clone()
    torch.diagonal(neg_dist_c_chem).fill_(float('inf'))
    torch.diagonal(neg_dist_chem_c).fill_(float('inf'))
    neg_dist_c_min_chem, _ = neg_dist_c_chem.min(dim=1)
    neg_dist_chem_min_c, _ = neg_dist_chem_c.min(dim=0)
    loss_c_chem_1 = F.relu(pos_dist_c_chem - neg_dist_c_min_chem + margin).mean()
    loss_c_chem_2 = F.relu(pos_dist_c_chem - neg_dist_chem_min_c + margin).mean()
    loss_c_chem = (loss_c_chem_1 + loss_c_chem_2) / 2

    # 组合所有损失项
    total_loss_triplet = loss_orig + loss_c_text + loss_c_chem

    # 正则化项
    reg_loss = lambda_param * (torch.norm(text_out, p=2) + torch.norm(chem_out, p=2) + torch.norm(f_out, p=2))

    total_loss = total_loss_triplet + reg_loss
    ...
    for epoch in range(epochs):
        ...
        for i, d in enumerate(training_generator):
            ...
            if MODEL == "StepwisePivotTransformer":
                ...
                # 调用新的损失函数，传入 f_c
                loss = bidirectional_triplet_loss(f_text, f_graph, f_c, margin=margin, lambda_param=lambda_param)
